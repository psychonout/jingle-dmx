    ...
    def apply(
        ...
        if strobe:
            ...
            if is_bass_beat:
                ...
                strobe.set_strobe(
                    self._cap(100, profile.max_strobe_level)
                )  # Reduced from 255
                strobe.set_color(90)
            else:
                strobe.set_dimmer(self._cap(180, profile.max_dimmer_level))
                strobe.set_strobe(
                    self._cap(80, profile.max_strobe_level)
                )  # Reduced from 255
        ...
        if eurolite_strobe:
            ...
            red = self._cap(
                int(frame.bass_energy * 255 * 0.4), profile.max_dimmer_level
            )
            green = self._cap(
                int(frame.mid_energy * 255 * 0.4), profile.max_dimmer_level
            )
            blue = self._cap(
                int(frame.high_energy * 255 * 0.4), profile.max_dimmer_level
            )
        ...
        if laser:
            laser.set_mode_level(
                self._cap(rng.randint(220, 255), profile.max_laser_level)
            )
            laser.pattern(rng.randint(128, 255))
            laser.color(rng.randint(0, 31))
        ...
        if stinger:
            if frame.rms >= thresholds.strobe_threshold:
                ...
                stinger.set_show_mode(0)  # Manual control
                stinger.set_color_macro("off")  # White moonflower
...
class FrequencyEffectStrategy(EffectStrategy):
    ...
    def _bass_heavy_effects(
        self, rms, laser, strobe, spotlight, stinger, eurolite_strobe, profile, rng
    ):
        ...
        if laser:
            laser.set_mode_level(
                self._cap(rng.randint(170, 220), profile.max_laser_level)
            )
            laser.color(rng.randint(0, 31))
        ...
        if stinger:
            ...
            stinger.set_uv_chase(
                self._cap(rng.randint(100, 127), profile.max_uv_level), strobing=False
            )
            stinger.set_laser_output(
    ...
    def _mid_heavy_effects(
        ...
        if laser:
            laser.set_mode_level(
                self._cap(rng.randint(140, 200), profile.max_laser_level)
            )
            laser.color(rng.randint(0, 31))
        ...
        if stinger:
            ...
            stinger.set_uv(self._cap(255, profile.max_uv_level))
            stinger.set_uv_chase(
                self._cap(rng.randint(110, 127), profile.max_uv_level), strobing=False
            )
            stinger.set_laser_output(
                strobe_speed=STINGER_LASER_BASE_SPEED, rotation_raw=135
            )  # Mid laser active
    ...
    def _high_heavy_effects(
        ...
        if strobe:
            ...
            strobe.set_strobe(
                self._cap(rng.randint(50, 75), profile.max_strobe_level)
            )  # Reduced
        ...
        if laser:
            laser.set_mode_level(
                self._cap(rng.randint(200, 255), profile.max_laser_level)
            )
            laser.color(rng.randint(0, 31))
        ...
        if stinger:
            ...
            stinger.set_led_strobe(
                self._cap(rng.randint(120, 200), profile.max_strobe_level)
            )
            stinger.set_uv(self._cap(255, profile.max_uv_level))
            stinger.set_uv_chase(
                self._cap(rng.randint(90, 127), profile.max_uv_level), strobing=True
            )
            stinger.set_laser_output(
                strobe_speed=STINGER_LASER_BOOST_SPEED,
...
class MegaComboEffectStrategy(EffectStrategy):
    ...
    def apply(
        ...
        if strobe:
            strobe.set_dimmer(self._cap(220, profile.max_dimmer_level))
            strobe.set_strobe(
                self._cap(120, profile.max_strobe_level)
            )  # Reduced from 255
            strobe.set_color(70)
            strobe.set_warm_white(self._cap(200, profile.max_strobe_level))
        if spotlight:
...
class ComboEffectStrategy(EffectStrategy):
    ...
    def apply(
        ...
        if laser:
            laser.set_mode_level(
                self._cap(int(170 + (intensity * 85)), profile.max_laser_level)
            )
            laser.color(rng.randint(0, 31))
...
class StrobeEffectStrategy(EffectStrategy):
    ...
    def apply(
        ...
        if laser:
            laser.set_mode_level(
                self._cap(int(100 + (effect_intensity * 155)), profile.max_laser_level)
            )
            laser.color(rng.randint(0, 31))
    ...
    def apply(
        ...
        if laser:
            ...
            if now - self._last_refresh > 1.25:
                ...
                self._cached_laser_color = rng.randint(5, 31)
                self._cached_laser_pattern = rng.randint(18, 42)
                self._last_refresh = now

            laser.set_mode_level(
                self._cap(
                    int(80 + (frame.rms / min_threshold) * 60), profile.max_laser_level
                )
            )
        ...
        if eurolite_strobe:
            ...
            eurolite_strobe.set_color(
                rng.randint(50, 150), rng.randint(50, 150), rng.randint(50, 150)
            )
    ...
    def apply(
        ...
        if laser:
            if frame.rms > min_threshold * 1.2:
                ...
                if now - self._last_refresh > 2.0:
                    self._cached_laser_color = rng.randint(5, 31)
                    self._cached_laser_pattern = rng.randint(4, 24)
                    self._last_refresh = now

                laser.set_mode_level(int(50 + (frame.rms / min_threshold) * 50))
                laser.color(self._cached_laser_color)
                laser.pattern(self._cached_laser_pattern)
                laser.speed(192)
            else:
                laser.set_mode_level(int(20 + (frame.rms / min_threshold) * 30))
        if stinger:
        ...
        if eurolite_strobe:
            if frame.rms > min_threshold * 1.2:
                ...
                eurolite_strobe.set_color(
                    rng.randint(20, 100), rng.randint(20, 100), rng.randint(20, 100)
                )
            else:
