...
def test_strobe_basic():
    ...
    logger.info("=== Basic Strobe Test ===")

    try:
        with Strobe() as strobe:
            logger.info("Strobe device opened successfully")

            # Test dimmer
            logger.info("Setting dimmer to maximum (255)")
            strobe.set_dimmer(255)
            time.sleep(1)

            # Test strobe effect
            logger.info("Setting strobe to simple strobe (80)")
            ...
            # Test warm white
            logger.info("Setting warm white to 255")
            strobe.set_warm_white(255)
            time.sleep(2)

            # Test cold white
            logger.info("Setting cold white to 255")
            strobe.set_cold_white(255)
            time.sleep(2)

            # Turn off strobe
            logger.info("Turning off strobe")
            strobe.set_strobe(0)
            strobe.set_dimmer(0)

            logger.info("✓ Basic strobe test completed successfully")
            return True
...
def test_strobe_channels():
    ...
    logger.info("=== Strobe Channel Test ===")
...
def test_strobe_effects():
    ...
    logger.info("=== Strobe Effects Test ===")
    ...
    try:
        with Strobe() as strobe:
            ...
            for value, description in effects:
                logger.info(f"Testing strobe effect: {description} (value: {value})")
...
def interactive_strobe_test():
    ...
    logger.info("=== Interactive Strobe Test ===")

    try:
        with Strobe() as strobe:
            logger.info("Strobe ready for interactive testing")
            logger.info("Commands:")
            logger.info("  dimmer <value>     - Set dimmer (0-255)")
            logger.info("  strobe <value>     - Set strobe (0-255)")
            logger.info("  color <value>      - Set color (0-255)")
            logger.info("  warm <value>       - Set warm white (0-255)")
            logger.info("  cold <value>       - Set cold white (0-255)")
            logger.info("  macro <value>      - Set macro (0-255)")
            logger.info("  speed <value>      - Set macro speed (0-255)")
            ...
            while True:
                try:
                    ...
                    if cmd == "quit":
                        ...
                        logger.info(f"✓ Macro speed set to {value}")
                    else:
...
def main():
    """Main test function"""
    logger.info("=== Strobe Debug Test Suite ===")
    ...
    logger.info(f"Channel test: {'✓ PASSED' if channel_success else '✗ FAILED'}")
    logger.info(f"Effects test: {'✓ PASSED' if effects_success else '✗ FAILED'}")
